...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.best_xgb_model = joblib.load('stamp_models/best_xgb_model.joblib')  # Load XGBoost model for stamp prediction
    ...
    def initUI(self):
        ...
        self.document_radio.setFont(QFont("Arial", 14))
        ...
        self.overlapping_radio.setFont(QFont("Arial", 14))  # Set font size to 14
        ...
        self.signature_validation_radio.setFont(QFont("Arial", 14))  # Set font size to 14

        self.stamp_validation_radio = QRadioButton("Check Stamp Validation", self)
        self.stamp_validation_radio.toggled.connect(self.toggleStampValidation)
        self.stamp_validation_radio.setFont(QFont("Arial", 14))  # Set font size to 14

        self.label = QLabel(self)
        self.label.setAlignment(Qt.AlignCenter)
        self.label.setFont(QFont("Arial", 16))  # Set font size to 16

        self.load_button = QPushButton(QIcon("icons/open.png"), "Load Image", self)
        self.load_button.clicked.connect(self.loadImage)
        self.load_button.setFont(QFont("Arial", 14))  # Set font size to 14
        ...
        self.process_button.setFont(QFont("Arial", 14))  # Set font size to 14
        ...
        radio_button_layout.addWidget(self.signature_validation_radio)
        radio_button_layout.addWidget(self.stamp_validation_radio)  # Added stamp validation radio button
    ...
    def resetImageDisplay(self):
        self.image_path = None
        self.image = None
        self.modified_image = None
        self.label.clear()  # Clear the label (remove any displayed image)
        self.load_button.setEnabled(True)  # Enable the "Load Image" button

    def toggleSignatureValidation(self):
        self.resetImageDisplay()
    ...
    def toggleStampValidation(self):
        self.resetImageDisplay()
        if self.stamp_validation_radio.isChecked():
            self.color_slider.setVisible(False)  # Hide color slider for stamp validation
        else:
            self.color_slider.setVisible(True)   # Show color slider for other processes
    ...
    def processImage(self):
        ...
        elif self.stamp_validation_radio.isChecked():  # Added check for stamp validation
            self.validateStamp()
    ...
    def validateStamp(self):
        if self.image_path is None:
            QMessageBox.warning(self, "Error", "Please load a stamp image first.")
            return
        ...
        if prediction[0] == 0:
            QMessageBox.information(self, "Stamp Validation", "The stamp is classified as REAL.")
        else:
            QMessageBox.information(self, "Stamp Validation", "The stamp is classified as FAKE.")
